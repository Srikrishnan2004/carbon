...
export async function getSubscriberPreferences({
  ...
  // Skip if using placeholder Novu key
  if (!NOVU_SECRET_KEY || NOVU_SECRET_KEY.includes("placeholder")) {
    console.warn("Novu not configured - skipping preference fetch");
    return { preferences: [] };
  }
...
export async function updateSubscriberPreference({
  ...
  // Skip if using placeholder Novu key
  if (!NOVU_SECRET_KEY || NOVU_SECRET_KEY.includes("placeholder")) {
    console.warn("Novu not configured - skipping preference update");
    return { success: true };
  }
